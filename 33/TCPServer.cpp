...
TCPServer::TCPServer(const char *ip, uint16_t port, int threadNum)
          : threadNum_(threadNum), acceptor_(&mainLoop_, ip, port), threadPool_(threadNum_, "IO"), mainLoop_(true)
{
    ...
    {
        subLoops_.push_back(std::make_unique<EventLoop>(false)); // 两种写法
        // subLoops_.emplace_back(new EventLoop(false));  // 创建从事件并放入容器中
...
void TCPServer::newConnection(std::unique_ptr<Socket> clientSock)
{
    ...
    // spConnection conn(new Connection(subLoops_[fd % threadNum_].get(), std::move(clientSock)));// 智能指针对象创建
