...
int main(int argc,char *argv[])
{
    ...
    ev.data.ptr=servchannel;          // 指定事件的自定义数据，会随着epoll_wait()返回的事件一并返回。这里把fd对应的Channel对象的指针传进去。`
    ...
    while (true)        // 事件循环。
    {
        ...
        {
            Channel *ch=(Channel *)evs[ii].data.ptr;    // 从事件的自定义数据中取出fd对应的Channel对象。
            // if (evs[ii].data.fd==listenfd)   // 如果是listenfd有事件，表示有新的客户端连上来。
            if ( ch->islisten()==true)   // 为什么这里用Channel对象的islisten_成员变量来判断，而不直接比较fd呢？因为在Reactor中，监听的fd和客户端连接的fd都可能有事件发生，如果直接比较fd，就无法区分是哪个fd了。通过Channel对象的islisten_成员变量，就可以区分是监听的fd还是客户端连接的fd了。
            {
                ////////////////////////////////////////////////////////////////////////
