...
int main(int argc, char *argv[])
{
    ...
    Channel *listenChannel = new Channel(&ep, listenSock.Sockfd()); // 创建一个Channel对象，参数是epoll对象和listenfd。
    ...
    while (true) // 事件循环。
    {
        ...
        {
            if (ch->getrevents() & EPOLLRDHUP) // 对方已关闭，有些系统检测不到，可以使用EPOLLIN，recv()返回0。
            {
                printf("client(eventfd=%d) disconnected.\n", ch->fd());
                close(ch->fd()); // 关闭客户端的fd。
            } //  普通数据  带外数据
            else if (ch->getrevents() & (EPOLLIN | EPOLLPRI)) // 接收缓冲区中有数据可以读。
            {
                ...
                if (ch == listenChannel) // 如果是listenfd有事件，表示有新的客户端连上来。listenfd只触发读事件
                {
                    ...
                    Channel *clientChannel = new Channel(&ep, clientSock->Sockfd()); // 用新连接的fd来构造一个Channel对象。
