...
Channel::Channel(Epoll *ep, int fd, bool islisten) : epoll_(ep), fd_(fd), islisten_(islisten)
{
}
...
void Channel::handleevents(Socket *serverSock)
{
    ...
    else if (revents_ & (EPOLLIN | EPOLLPRI)) // 接收缓冲区中有数据可以读。
    {
        ...
        if (islisten_ == true) // 如果是listenfd有事件，表示有新的客户端连上来。listenfd只触发读事件
        {
            ...
            Channel *clientChannel = new Channel(epoll_, clientSock->Sockfd(), false); // 用新连接的fd来构造一个Channel对象。
