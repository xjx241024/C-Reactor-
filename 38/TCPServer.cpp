...
void TCPServer::removeTimeoutConn(int fd)
{
    ...
    if (removeTimeoutConnCallback_)
        removeTimeoutConnCallback_(fd); // 回调EchoServer::handleRemoveTimeoutConn
}
...
void TCPServer::setRemoveTimeoutConnCallback(std::function<void(int)> cb)
{
    removeTimeoutConnCallback_ = cb;
}