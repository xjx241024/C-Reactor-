...
EventLoop::EventLoop(bool isMainLoop, int timeInterval, int timeOut) : epoll_(new Epoll()), isMainLoop_(isMainLoop), isStop_(false),
                                                                       timeInterval_(timeInterval), timeOut_(timeOut),
...
{
    isStop_ = true;
    wakeUp();   // 唤醒事件循环，否则事件循环会在下次闹钟响时，或epoll_wait()超时才停下来
}
...
void EventLoop::handleTimer()
{
    ...
    struct itimerspec timeout; // 定时时间的数据结构。
    memset(&timeout, 0, sizeof(struct itimerspec));
    timeout.it_value.tv_sec = timeInterval_; // 定时时间。
    timeout.it_value.tv_nsec = 0;
    ...
    else
    {
        ...
        {
            // printf(" %d", it->first);
            if (it->second->timeOut(now, timeOut_)) // 判断是否超时
            {
                printf("删除超时的连接fd, 其线程ID= %ld.\n", syscall(SYS_gettid));
