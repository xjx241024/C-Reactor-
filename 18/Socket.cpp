...
{
    ip_ = serveraddr.ip();     // 将InetAddress对象的ip()函数返回的字符串表示的IP地址赋值给成员变量ip_。
    port_ = serveraddr.port(); // 将InetAddress对象的port()函数返回的整数表示的端口号赋值给成员变量port_。
...
{
    ip_ = clientaddr.ip();     // 将InetAddress对象的ip()函数返回的字符串表示的IP地址赋值给成员变量ip_。
    port_ = clientaddr.port(); // 将InetAddress对象的port()函数返回的整数表示的端口号赋值给成员变量port_。
