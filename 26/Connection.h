...
private:
    ...
    std::function<void(Connection *, std::string &)> onMessageCallback_; // 处理消息的回调函数，参数是一个无参数、无返回值的函数对象。
    std::function<void(Connection *)> sendComplteCallback_; // 发送完成的函调函数
...
public:
    ...
    void setOnMessageCallback(std::function<void(Connection *, std::string &)> cb); // 设置处理消息的回调函数，参数是一个无参数、无返回值的函数对象。
    void setSendCompleteCallback(std::function<void(Connection *)> cb); // 处理发送完成事件的回调函数

    void onMessage(); // 处理消息的函数
    void send(const char *msg, size_t len); // 发送消息的函数，参数是要发送的消息字符串。
