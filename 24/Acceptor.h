...
private:
    ...
    std::function<void(Socket*)> newConnectionCallback_; // 处理新连接的回调函数，表示一个参数为Socket*、无返回值的函数对象。
...
public:
    ...
    void setNewConnectionCallback(std::function<void(Socket*)> cb); // 设置处理新连接的回调函数。
};