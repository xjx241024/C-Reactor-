...
void Connection::setOnMessageCallback(std::function<void(Connection *, const std::string &)> cb)
{
    ...
    while (true)
    {
        ...
        else if (nread == -1 && ((errno == EAGAIN) || (errno == EWOULDBLOCK))) // 全部的数据已读取完毕。
        {
            ...
            while (true)
            {
                ...
                onMessageCallback_(this, content); // 调用处理消息的回调函数。
                //     content = "Reply: " + content; // 构造一个回复报文。
                //     len = content.size();
                //     std::string sendBuffer((char*)&len, 4); // 构造一个临时发送缓冲区，前4个字节存放报文内容的长度，后面存放报文内容。
                //     sendBuffer.append(content); // 把回复报文的内容添加到发送缓冲区。
                //     send(fd(), sendBuffer.data(), sendBuffer.size(), 0); // 把发送缓冲区的数据发送回去。
            }
            break;
        }
...
{
    outputBuffer_.append(msg, len);      // 把要发送的数据添加到输出缓冲区。
...
void Connection::writeCallback()
{
    ...
    if (outputBuffer_.size() == 0) // 发送缓冲区没有数据，表示已经发送成功了，不再关注写事件了。
