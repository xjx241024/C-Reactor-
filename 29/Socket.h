...
private:
    ...
    const char* ip_; // 如果是listenfd，存放服务端监听的IP，如果是客户端fd，存放客户端连接的IP
    uint16_t port_; // 如果是listenfd，存放服务端监听的端口，如果是客户端fd，存放客户端连接的端口
public:
    Socket(int sockfd); // 构造函数

    int Sockfd() const; // 获取套接字文件描述符的成员函数
    const char* ip() const; // 获取字符串表示的IP地址字符串。
    uint16_t port() const;  // 获取整数表示的端口号，返回值是主机字节序的端口号。
    void setipport(const char* ip, uint16_t port); // 设置IP地址和端口号，直接赋值给成员变量ip_和port_。
