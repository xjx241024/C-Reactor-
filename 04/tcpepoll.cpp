...
int main(int argc, char *argv[])
{
    ...
    ev.data.fd = listenSock.getSockfd(); // 指定事件的自定义数据，会随着epoll_wait()返回的事件一并返回。
    ev.events = EPOLLIN;   // 让epoll监视listenfd的读事件，采用水平触发。

    epoll_ctl(epollfd, EPOLL_CTL_ADD, listenSock.getSockfd(), &ev); // 把需要监视的listenfd和它的事件加入epollfd中。
    ...
    while (true) // 事件循环。
    {
        ...
        for (int ii = 0; ii < infds; ii++) // 遍历epoll返回的数组evs。
        {
            ...
            {
                // listenfd只触发读事件。
                if (evs[ii].data.fd == listenSock.getSockfd()) // 如果是listenfd有事件，表示有新的客户端连上来。
                {
                    ...
                    epoll_ctl(epollfd, EPOLL_CTL_ADD, clientSock->getSockfd(), &ev);
                }
                else
                {
                    char buffer[1024];
